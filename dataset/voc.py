...
class VOCSegmentation(data.Dataset):
    ...
    def __init__(self, root, image_set='train', is_aug=True, transform=None):
        ...
        self.root = os.path.expanduser(root) # './data/PascalVOC12'
        self.year = "2012"
        ...
        if labels is not None:
            ...
            if train:
                if len(labels) == 1:
                    ...
                    select_labels_num = int(len(labels) * class_ratio)
                    tmp_labels = random.sample(labels, select_labels_num) 

                    # if idxs_path is not None and os.path.exists(idxs_path):
                    #     idxs = np.load(idxs_path).tolist()
                    # else:
                    idxs = filter_images(full_voc, tmp_labels, labels_old, overlap=overlap)
                    idxs = random.sample(idxs, int(len(idxs)*(sample_ratio2))) 
                        # if idxs_path is not None and distributed.get_rank() == 0:
                        #     np.save(idxs_path, np.array(idxs, dtype=int))
            else:
                # if idxs_path is not None and os.path.exists(idxs_path):
