...
class BCEWithLogitsLossWithIgnoreIndexSoftLabel(nn.Module):
    ...
    def forward(self, inputs, old_outputs, old_classes, targets):
        ...
        targets = F.one_hot(labels_new, inputs.shape[1] + 1).float().permute(0, 3, 1, 2)
        targets = targets[:, :inputs.shape[1], :, :]  # remove 255 from 1hot
        '''outputs_mib = torch.zeros_like(inputs)  # B, C (1+V+N), H, W
        den = torch.logsumexp(inputs, dim=1)  # B, H, W       den of softmax
        outputs_mib[:, 0] = torch.logsumexp(inputs[:, 0:old_classes], dim=1) - den  # B, H, W       p(O)
        outputs_mib[:, old_classes:] = inputs[:, old_classes:] - den.unsqueeze(dim=1)  # B, N, H, W    p(N_i)
        loss1 = F.nll_loss(outputs_mib, targets, ignore_index=self.ignore_index, reduction=self.reduction)'''
        ...
        #loss2 = F.nll_loss(outputs, labels, ignore_index=self.ignore_index, reduction=self.reduction)
        #mask_valid = mask_background & mask_valid_pseudo
        #mask_foreground = ~mask_background
        #mask = mask_valid | mask_foreground
        #loss = -(outputs * labels).mean(dim=1) * mask
        loss = -(outputs * labels).mean(dim=1)
        #loss = F.kl_div(outputs, labels, reduction='mean')
        #loss = loss1 + loss2
        return loss
