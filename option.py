...
def modify_command_options(opts):
    ...
    if opts.incremental_method is not None:
        if opts.incremental_method == 'FT':
            pass
        if opts.incremental_method == 'LWF':
            opts.loss_kd = 100
        if opts.incremental_method == 'ILT':
            opts.loss_kd = 100
            opts.loss_de = 100
        if opts.incremental_method == 'MiB':
            opts.unce = True
            opts.unkd = True
            opts.init_balanced = True
        if opts.incremental_method == 'PLOP':
            opts.pod = "local"
        ...
        if opts.incremental_method == 'RCIL':
            opts.pod = "local"
            opts.pod_factor = 0.01
            opts.pod_logits = True
            opts.pod_options = {"switch": {"after": {"extra_channels": "sum", "factor": 0.0005, "type": "local"}}}
        ...
        if opts.incremental_method == 'OURS':
            opts.pod = "local"
            opts.pod_factor = 0.01
            opts.pod_logits = True
            opts.pod_options = {"switch": {"after": {"extra_channels": "sum", "factor": 0.0005, "type": "local"}}}
...
def args_parser():
    ...
    parser.add_argument('--sample_ratio1', type=float, default=0.6, help='ratio of data for local clients (class=1)')
    parser.add_argument('--batch_size', type=int, default=24, help='size of mini-batch')  
    parser.add_argument('--epochs_local', type=int, default=6, help='local epochs of each global round')  
    parser.add_argument('--lr1', type=float, default=0.01, help='learning rate step=0')
    parser.add_argument('--lr2', type=float, default=0.001, help='learning rate step>0')
    ...
    parser.add_argument("--max_portion", type=float, default=0.8)
    parser.add_argument("--portion_step", type=float, default=0.1) 
    parser.add_argument("--soft_factor", type=float, default=1.0)
    ...
    parser.add_argument('--opt_level', type=str, choices=['O0', 'O1', 'O2', 'O3'], default='O1')
    ...
    parser.add_argument(
        "--pseudo_proto", default=False, action="store_true", help="apply output distillation."
    )
