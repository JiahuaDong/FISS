...
class Trainer_rcil:
    ...
    def before(self, train_loader):
        if self.pseudo_labeling is None:
            return
        if self.pseudo_labeling.split("_")[0] == "median" and self.step > 0 and self.model_old is not None:
            if self.rank==0:
                print("Find median score")
            self.thresholds, _ = self.find_median(train_loader, self.device)
        elif self.pseudo_labeling.split("_")[0] == "entropy" and self.step > 0 and self.model_old is not None:
            if self.rank==0:
                print("Find median score")
            self.thresholds, self.max_entropy = self.find_median(
                train_loader, self.device, mode="entropy"
            )
    ...
    def find_median(self, train_loader, device, mode="probability"):
        ...
        for cur_step, (images, labels) in enumerate(train_loader):
            ...
            if cur_step % 10 == 0:
                if self.rank==0:
                    print(f"Median computing {cur_step}/{len(train_loader)}.")
        ...
        if self.rank==0:
            print(f"Finished computing median {thresholds}")
